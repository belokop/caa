...
function getSize() {
	...
	rm -rf /tmp/t.img
	cp -p "$@" /tmp/t.img
	imageH=`identify /tmp/t.img 2>/dev/null|awk '{print $3}'|awk -Fx '{print $2}'`
	imageW=`identify /tmp/t.img 2>/dev/null|awk '{print $3}'|awk -Fx '{print $1}'`
	mimeType=`identify /tmp/t.img 2>/dev/null|awk '{print $2}'|tr A-Z a-z`
    fi
}
